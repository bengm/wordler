    ...
    guesses.forEach((g) => {
      g.forEach((l) => {
        if (l.letter.length > 0) {
          wordList = Filter[l.status](wordList, l.position, l.letter);
        }
      });
    });
    return wordList;
...
export const Form = {
  ...
  letterInput: (guessNum, position, state) => {
    let lTabIndex = 1 + guessNum * 2;
    let bTabIndex = lTabIndex + 1;
    return `
      <div class="letter">
        <div class="form-group">
          <label for="letter_${guessNum}_${position}"></label>
          <input  id="letter_${guessNum}_${position}" value="${state.letter}" type="text" tabindex="${lTabIndex}" maxlength="1" class="letter" />
          <button id="result_${guessNum}_${position}" value="${state.status}" type="text" tabindex="${bTabIndex}" class="btn result gray">gray</button>
        </div>
      </div>
    `;
  },
  ...
  setResultListeners: () => {
    ...
    Data.guesses.forEach((g) => {
      g.forEach((l) => {
        document
          .getElementById(`result_${l.guessNum}_${l.position}`)
          .addEventListener("click", Form.toggleResult, false);
        document
          .getElementById(`letter_${l.guessNum}_${l.position}`)
...
export const Score = {
  byLetterFrequency: (matchedWords, allWordsData, letterFreq, guesses) => {
    ...
    allWordsData.sort((a, b) => b.lfGuessScore - a.lfGuessScore);
  },
